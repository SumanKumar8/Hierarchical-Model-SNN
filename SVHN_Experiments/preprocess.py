...
output_files = os.listdir('Output_spike_train_svhn/')
output_files = [x for x in output_files if x.endswith('.txt')]
output_files.sort(key=natural_keys)

target_files = os.listdir('target_train_svhn/')
...
for op_file, tr_file in zip(output_files, target_files[:len(output_files)]):
    ...
    hw_preds = process_spikes('Output_spike_train_svhn/' + op_file,reverse=True, n_classes = n_classes).numpy().tolist()
    hw_pred += hw_preds
    gt = open('target_train_svhn/' + tr_file).readlines()
...
input_files = os.listdir('SpikeGenData_train_svhn/')
...
for ip_file in input_files[:len(output_files)]:
    print("File:", ip_file)
    ip_file = open('SpikeGenData_train_svhn/' + ip_file, 'r').readlines()
...
for op_file in output_files:
    print(op_file)
    hw_op = open('Output_spike_train_svhn/' + op_file, 'r').readlines()
...
# Selecting only correctly classified ground truths
...
for cls in classes:
  ...
  file_name = "data_svhn_cls_" + str(cls) + ".npy"
  ...
  file_name1 = "target_svhn_cls_" + str(cls) + ".npy"
