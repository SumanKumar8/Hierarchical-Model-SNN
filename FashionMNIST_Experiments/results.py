...
output_files = os.listdir('Output_spike_test_inject_Fashion_mnist/')
output_files = [x for x in output_files if x.endswith('.txt')]
output_files.sort(key=natural_keys)

input_files = os.listdir('SpikeGenData_test_Fashion_mnist/')
...
for ip_file in input_files[:len(output_files)]:
    print("File:", ip_file)
    ip_file = open('SpikeGenData_test_Fashion_mnist/' + ip_file, 'r').readlines()
...
for op_file in output_files:
    print(op_file)
    hw_op = open('Output_spike_test_inject_Fashion_mnist/' + op_file, 'r').readlines()
...
for model_name in regressors_name:
